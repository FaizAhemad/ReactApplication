    ...
    if (imageReg.test(file.originalName)) {
      if (imageReg.test(file.detectedFileExtension)) {
        ...
        const findUserByEmail = await sql.query(
          `select * from tblusers where email='${req.body.email}'`,
        );
        ...
        if (user) {
          ...
          sql.query(
            `INSERT INTO tblusers (email,password,genderid,firstname, middlename, lastname,
                        fullname,dob,country,state,city,address,mobile,image) VALUES ('${email}','${hashedPassword}','${
              gender === 'Male' ? 1 : 2
            }','${firstname}','${middlename}','${lastname}',
                        '${firstname} ${middlename} ${lastname}','${day}-${month}-${year}','${country}','${state}','${city}',
                        '${address}','${mobile}','${file.originalName}')`,
            async function (err, result) {
              if (err) throw err;
              await pipeline(
                file.stream,
                fs.createWriteStream(
                  `${__dirname}/../../public/images/user/${filename}`,
                ),
              );
        ...
        res.status(200).send({
          message: `The images you chosen is corrupted and is not valid, Please choose different image file.`,
        });
      }
    } else {
      res.status(200).send({
        message: `Please choose an image file. It seems you selected ${extension.replace(
          '.',
          '',
        )} file.`,
      });
