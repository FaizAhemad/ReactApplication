...
router.get('/fetchcategories', async (req, res) => {
    try {
        const fetchAllCategoryQuery = await sql.query("select * from tblcategories");
        const categories = fetchAllCategoryQuery.recordset;
        const fetchAllSubcategoriesQuery = await sql.query("select * from sub_categories JOIN tblcategories ON sub_categories.category = tblcategories.id");
        const subcategories = fetchAllSubcategoriesQuery.recordset;
        let data = categories.map((category) => {
            category.subcategories = [];
            subcategories.map((subcategory) => {
                if (category.cat_name === subcategory.cat_name) {
                    category.subcategories = [...category.subcategories, subcategory.sub_categoryname];
                }
            })
            return category;
        });
