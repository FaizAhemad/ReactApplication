...
function Home({
  ...
  const {currentPage} = props.store.productsReducer.pagination;
  const pageLimit = 1;
  const {totalPages} = props.store.productsReducer.pagination;
  const {data} = props.store.productsReducer.pagination;
  ...
  const array = data.slice(indexoffirstpost, indexoflastpost);
    ...
    <>
      ...
      >
        ...
        <Card style={{margin: '5px', flexBasis: '330px'}}>
          <div
            style={{
              display: currentView === 'column' && 'flex',
              width: currentView === 'column' && '100%',
              justifyContent: currentView === 'column' && 'center',
            }}
          >
            <Card.Img
              variant="top"
              src="https://th.bing.com/th/id/OIP.ibsxlWfu5ieS5zx20UVuuAHaF3?w=226&h=180&c=7&r=0&o=5&dpr=1.5&pid=1.7"
              style={{
                height: '300px',
                width: currentView === 'column' ? '300px' : '',
              }}
            />
          </div>
          <Card.Body>
        ...
        >
          ...
          <div>
            ...
            >
              ...
              <input
                className="page-link"
                style={{marginTop: '-16px', width: '95px', marginLeft: '10px'}}
                ...
                onBlur={e => {
                  ...
                  if (re1.test(e.target.value)) {
                    if (parseInt(e.target.value) > totalPages) {
                      ...
                      props.setPaginationProps({
                        currentPage: parseInt(e.target.value),
                      });
                    }
                  } else {
                    setCurrentPageValue(currentPage);
                    props.setPaginationProps({currentPage});
                  }
                }}
              />
              <div>
                <p className="page-link " style={{border: 'none'}}>
                  {constants.SHOWING} {currentPage}{' '}
                  {constants.OF.toLocaleLowerCase()} {totalPages}{' '}
                  {constants.PAGES.toLocaleLowerCase()}
                </p>
              </div>
            </div>
          </div>
